...
public class Snake1WO extends GameType {
	...
	public Snake1WO(JFrame frame, Menu menu, GamePlay game){
		this.frame = frame;
		this.menu = menu;
		opp = new Snake[game.noOpp];
	...
	public void tick() {
		...
		if(!snake.move()){
			game.getTimer().stop();
			new DialogBox(score, Mode.Snake1WO, frame, menu, game);
		} else {
			if(snake.eats(fruit)) {
				snake.lengthen();
	...
	public void start() {
		fruit = new Fruit(game.grid.getFree());
		snake = new Snake1(game.grid, game.grid.grid[game.width/2][game.height/2]);
		for (int i = 0; i < game.noOpp; i++){
			opp[i] = new Snake1(game.grid, game.grid.grid[game.width/2][game.height/(3 + i)]);
		}
